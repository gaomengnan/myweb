...
<a class="btn btn-primary" href="<?php echo U('Index/index','','');?>"> 返回</button>
    ...
    <div id="myTabContent" class="tab-content">
     ...
     <form id="tab">
      <label>编号</label>
        ...
        <input type="text" name="id"  value="<?php echo ($res["id"]); ?>" class="input-xlarge" readonly>

        <label>姓名</label>

        <input type="text" name="username"  value="<?php echo ($res["username"]); ?>" class="input-xlarge">
...
<label>密码</label>
        ...
        <input type="text" name="password" value="<?php echo ($res["password"]); ?>"  class="input-xlarge">


        </textarea>

        <label>选择身份</label>
<input value="管理员" name="status" type="radio">管理员
<input value="普通用户" name="status" type="radio" checked>普通用户
...
if(confirm('确定要修改吗')){
var url = '<?php echo U('Index/save','','');?>';
//alert(1);
var username = $('input[name="username"]');
//alert(username);
var password = $('input[name="password"]');
var sta   = $('input[name="status"]:checked');

var id = $('input[name="id"]');
$.post(url,{username:username.val(),password:password.val(),status:sta.val(),id:id.val()},function(data){
             if(data.success==1){
